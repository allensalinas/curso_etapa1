 ...
 */
    ...
    static sincroPendingFavorite() {
        // Fetch all restaurants
        let pendientes = cargarPendientesFavorito().then(
            function(pendientes){
                pendientes.forEach(restaurant => {
                fetch(DBHelper.getChangeFavoriteURL(restaurant.id, restaurant.is_favorite), {
                    method: 'put'
                }).then(function (response) {
                    if (response.ok) { //Creado
                        restaurant.pendingfavorite = 'false';
                        updateFavorite(restaurant);
                    } else {
                        restaurant.pendingfavorite = 'true';
                        updateFavorite(restaurant);
                    }
                })
                .catch(function (errorResponse) {
